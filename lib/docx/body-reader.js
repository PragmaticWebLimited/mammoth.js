...
function BodyReader(options) {
    ...
    // Returns true if the element has a non-zero height border, otherwise false
    function readBorderBottom(element) {
        var borderBottom = element.first('w:bottom');
        // Check <w:bottom> child node exists
        if (borderBottom) {
            var space = borderBottom.attributes["w:space"];
            // Check the 'w:space' attribute value not "0", i.e. the border has a height
            return space !== "0";
        } else {
            return false;
        }
    ...
    // Returns true if the element is a "separator" paragraph, otherwise false
    function readParagraphSeparator(element) {
        var paragraphBorderElement = element.first('w:pBdr');
        // Check <w:pBdr> (paragraph border) child node exists
        if (paragraphBorderElement) {
            // Check for non-zero height border
            return readBorderBottom(paragraphBorderElement);
        }
        ...
        "w:pPr": function(element) {
            return readParagraphStyle(element).map(function(style) {
                return {
                    ...
                    isSeparator: readParagraphSeparator(element)
                };
            });
        },
    ...
    function readTableCell(element) {
        return readXmlElements(element.children).map(function(children) {
            ...
            var alignment = children[0].alignment || null;

            var cell = documents.TableCell(children, {colSpan: colSpan, alignment: alignment});

...
function DocumentConversion(options, comments) {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        // children.length check as separator xml can appear in paragraphs with
        // content (and we don't want to break them).
        if ( element.isSeparator && element.children.length === 0 ) {
        ...
        if (style) {
            ...
            if ( element.styleId ) {
                ...
                if ( ['GraphTitle', 'Subtitle'].includes( element.styleId ) ) {
                    style = htmlPaths.topLevelElement("h4");
                }
            }

            if ( element.footerReferences ) {
                style = htmlPaths.topLevelElement("pwl:directive");
            }
    ...
    function convertRun(run, messages, options) {
        ...
        // Default empty string (falsey) - fallback for <br/>
        var runContent = '';
        if ('undefined' !== typeof run.children[0] && 'value' in run.children[0]) {
            // Also an empty string if this run contains ONLY whitespace.
            runContent = run.children[0].value.trim();
        }

        // Don't set HTML tags (<strong>, etc) if this run contains ONLY 
        // whitespace or <br/>, since Gutenberg strips these "empty" tags.
        if (runContent) {
            if (run.isSmallCaps) {
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (element.alignment !== null) {
            attributes.class = 'tcell-align-' + element.alignment.toString();
        }
